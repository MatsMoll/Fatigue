...
class ActivityRecorder: ObservableObject {
    ...
    // - Advanced Models
    @Published
    var lsctResult: LSCTResult?
    
    private var lsctComputation: LSCTResultStreamComputation?
    private var lsctWorkout: Workout? { lsctComputation?.workout }
    
    private let dfaModel: DFAStreamModel
    ...
    init(settings: UserSettings, deviceManager: DeviceManager = .init()) {
        self.settings = settings
        self.deviceManager = deviceManager
        self.dfaModel = DFAStreamModel(windowDuration: settings.dfaWindow)
        self.prevRawFrame = .init(timestamp: 0, power: nil, heartRate: nil, cadence: nil)
        recordedFrames = []
        recordedLaps = []
        elapsedTime = 0
        elapsedRoundTime = 0
    ...
    func enableLsctDetection(workoutStore: WorkoutStore) {
        guard let ftp = settings.ftp, let baselineWorkout = workoutStore.baseline else { return }
        lsctComputation = LSCTResultStreamComputation(
            workout: .init(
                id: .init(),
                startedAt: .init(),
    ...
    func resetRecorder() {
        ...
        latestFrame = startFrame.frame
        startedAt = nil
        lsctComputation?.reset()
    }
    
    private func recordFrame() {
        let rawFrame = deviceManager.currentValues.frame(timestamp: elapsedTime)
        let processedFrame = process(rawFrame)
        let newFrame = processedFrame.fill(with: prevRawFrame)
        prevRawFrame = processedFrame
        ...
        elapsedRoundTime += 1
        deviceManager.currentValues = .init()
        updateLsctDetector(with: newFrame)
    }
    
    func process(_ frame: WorkoutFrame) -> WorkoutFrame {
        updateDfaModel(with: frame)
        
        var processedHeartRate: WorkoutFrame.HeartRate?
        if let heartRate = frame.heartRate {
            processedHeartRate = .init(
                value: heartRate.value,
                rrIntervals: heartRate.rrIntervals,
                dfaAlpha1: try? dfaModel.compute().beta
            )
        }
    ...
    func updateDfaModel(with frame: WorkoutFrame) {
        guard let rrIntervals = frame.heartRate?.rrIntervals else { return }
        for value in rrIntervals {
            dfaModel.add(value: value)
        }
    }
    
    func updateLsctDetector(with frame: WorkoutFrame) {
        guard let lsctComputation = lsctComputation else { return }
        Task {
            do {
                lsctWorkout?.frames.append(frame)
                let lsctResult = try await lsctComputation.compute()
                guard self.lsctResult?.runIdentifer.startingAt != lsctResult.runIdentifer.startingAt else { return }
                DispatchQueue.main.async { [weak self] in
                    self?.lsctResult = lsctResult
                }
            } catch {
                print("Error: \(error)")
            }
